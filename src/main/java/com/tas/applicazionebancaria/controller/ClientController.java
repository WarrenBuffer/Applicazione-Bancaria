...
public class ClientController {
	...
	private static String validateInputs(Cliente cliente) {
		if (!cliente.getNomeCliente().matches("^[a-zA-Z ,.'-]{2,30}$") || cliente.getNomeCliente().trim().isEmpty() || cliente.getNomeCliente()==null) 
			return "Il campo nome non può essere vuoto e deve contenere solo lettere";
		if (!cliente.getCognomeCliente().matches("^[a-zA-Z ,.'-]{2,30}$") || cliente.getCognomeCliente().trim().isEmpty() || cliente.getCognomeCliente()==null) 
			return "Il campo cognome non può essere vuoto e deve contenere solo lettere";
		if (!cliente.getEmailCliente().matches("^[\\w.%+-]+@[a-zA-Z0-9.-]+\\.[a-zA-Z]{2,}$") || cliente.getEmailCliente().trim().isEmpty() || cliente.getEmailCliente()==null) 
			return "Il campo email non può essere vuoto e deve essere un indirizzo email valido";
		if (!cliente.getPasswordCliente().matches("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#&%^$?=])[a-zA-Z0-9@#&%^$?=]{7,15}$") || cliente.getPasswordCliente().trim().isEmpty() || cliente.getPasswordCliente()==null) 
	...
	public ModelAndView controlloLogin(@RequestParam("email") String email, @RequestParam("password") String password, HttpServletResponse response, HttpServletRequest request) {
		...
		if(c.isPresent()) {
			...
			if(encoder.matches(password, c.get().getPasswordCliente())) {
				String token = JWT.generate(c.get().getNomeCliente(), c.get().getCognomeCliente(), c.get().getEmailCliente());
